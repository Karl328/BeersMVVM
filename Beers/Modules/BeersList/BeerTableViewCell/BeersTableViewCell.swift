...
final class BeersTableViewCell: UITableViewCell {
    ...
    var viewModel: BeersTableViewCellViewModelProtocol! {
        didSet {
            beerNameLabel.text = viewModel.beerName
            imageBeer.kf.setImage(with: URL(string: viewModel.imageURL))
            brewedLabel.text = viewModel.brewedName
            foodPairingLabel.text = viewModel.foodPairing
            abvLabel.text = viewModel.abvNumber
        }
    }
